...
object Interaction2 extends Interaction2Interface {
  ...
  def availableLayers: Seq[Layer] = List(
    Layer(Temperatures, temperaturesColorScale, 1975 to 2015),
    Layer(Deviations, deviationsColorScale, 1991 to 2015))
  ...
  def yearSelection(selectedLayer: Signal[Layer], sliderValue: Signal[Year]): Signal[Year] = {
    Signal({
      ...
      if (bounds.min >= year) bounds.min
      else if (bounds.max <= year) bounds.max
      else sliderValue()
    })
  }
